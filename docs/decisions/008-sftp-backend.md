...
- 沒有 per-prefix 權限（見 format.md §10）；`remove` 不能列進黑名單。實測過（`TestSFTPRemoveBlacklistLeavesSpoolFilesBehind`，第二個容器用 `/etc/sftp.d/` 腳本把 `ForceCommand` 改成 `internal-sftp -P remove`）：`PutIfAbsent` 成功、`.tmp-*` 留下、`Delete` 回 access denied。
