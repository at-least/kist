...
### 8. 誠實地寫下 M2 證明了什麼、沒證明什麼

- 快取省的是**穩態**的記憶體（查詢不載入整張表）與網路（只抓新 blob）；但**更新與重建時**
  仍把所有紀錄放進一個 `Vec` 排序再寫回。M5 的目標（100 萬檔 < 512 MiB）要盯這個數字；
  解法是舊表（已排序）與新紀錄（排序後）的串流合併，不需要改格式。
- ADR 003 §1 說「M2 有本地快取後用 cache_id 記住寫過的 tree 就能省掉每次 put」——**M2 沒做**，
  每次 backup 仍然 put 每個 tree。列在下面「沒做」。
- 並發測試是同一個 process 裡用 `tokio::join!` 交錯兩個 future，加上本機重複 5 輪；
  PLAN 要的「proptest 產生交錯順序」與跨 process 的競態測試是 M3（GC 的刪除才是危險的那一半）。
