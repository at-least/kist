...
pub fn obtain(password_file: &Option<PathBuf>, confirm: bool) -> Result<String> {
    if let Some(path) = password_file {
        let content = std::fs::read_to_string(path)
            .with_context(|| format!("cannot read password file {}", path.display()))?;
        let first = content.lines().next().unwrap_or("").to_owned();
        if first.is_empty() {
    ...
    let pw = rpassword::prompt_password("repository password: ")
